...
@section('content')
    <h1 class="text-2xl text-center mt-10">Nueva oferta</h1>

    <form class="max-w-lg mx-auto my-10" action="{{route('vacantes.store')}}" method="POST" novalidate>
        @csrf
        <div class="mb-5">
            <label for="titulo" class="block text-gray-700 text-sm mb-2">{{ __('Titulo de la Oferta') }}</label>
            <input id="titulo" type="text" class="p-3 bg-white-100 rounded form-input w-full @error('titulo') @enderror" name="titulo" autocomplete="current-titulo" placeholder="Titulo de la oferta" value="{{old('titulo')}}">
                @error('titulo')
